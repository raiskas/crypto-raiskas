...
async function refreshAll() {
  try {
    const [live, summary, sweep, news, tradesAll] = await Promise.all([
      getJson("/api/live"),
      getJson("/api/backtest-summary"),
      getJson("/api/backtest-sweep"),
      getJson("/api/global-news"),
      getJson("/api/recent-trades?symbol=&limit=200"),
    ]);
    ...
    const trades = await getJson(`/api/recent-trades?symbol=${encodeURIComponent(symbol)}&limit=50`);
...
async function runRobotRefresh(trigger = "manual") {
  ...
  try {
    await postJson("/api/refresh-run");
  } catch (err) {
  ...
  const timer = setInterval(async () => {
    tries += 1;
    try {
      const st = await getJson("/api/refresh-status");
      const s = st.status || {};
      if (s.running) {
