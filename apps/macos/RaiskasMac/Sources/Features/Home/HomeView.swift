...
final class HomeViewModel: ObservableObject {
  ...
  func carregarTopMoedas() async {
    ...
    errorTopMoedas = nil
    do {
      let fetched = try await SupabaseService.shared.fetchMarketTickers(ids: top10Ids)
      topMoedas = fetched
    } catch {
      errorTopMoedas = "Mercado instável. Exibindo último snapshot disponível."
      if topMoedas.isEmpty {
        topMoedas = []
      }
    }
      ...
      VStack(alignment: .leading, spacing: 16) {
        ...
        VStack(alignment: .leading, spacing: 4) {
          Text("Bem-vindo, \(username)")
            .font(.system(size: 34, weight: .bold))
            .foregroundStyle(AppTheme.strongText)
          Text("Aqui está um resumo do seu portfólio.")
        ...
        HStack(spacing: 12) {
          HomeMetricCard(
            ...
            subtitle: "Valor atual de mercado",
            icon: "wallet.pass"
          )
          HomeMetricCard(
            title: "Custo Base Total",
            value: totalCostValue,
            subtitle: "Custo total de aquisição",
            icon: "creditcard"
          )
          HomeMetricCard(
          ...
          HomeMetricCard(
            title: "L/P Realizado",
            value: realizedValue,
            subtitle: "Ganhos/Perdas de vendas",
      ...
      VStack(alignment: .leading, spacing: 12) {
        ...
        if loading {
          ...
          Table(rankedMoedas) {
            ...
            TableColumn("Cap. de Mercado") { row in
              Text("$ \(AppFormatters.currency(row.ticker.marketCap).replacingOccurrences(of: "$", with: ""))")
