...
import type { CryptoPerformanceState, FullCoinData, MarketDataMap, PerformanceSummary } from "@/types/crypto";
...
const ensureJsonApiResponse = async (response: Response) => {
...
export default function HomePage({ initialData }: HomePageProps) {
  ...
  const [loadingPerformance, setLoadingPerformance] = useState(!initialData?.performanceData && !initialData?.errorPerformance);
  const [errorPerformance, setErrorPerformance] = useState<string | null>(initialData?.errorPerformance ?? null);

  const [topMoedas, setTopMoedas] = useState<TopMoeda[]>(initialData?.topMoedas ?? []);
  const [loadingTopMoedas, setLoadingTopMoedas] = useState(
    !initialData?.topMoedas?.length && !initialData?.errorTopMoedas
  );
  const [errorTopMoedas, setErrorTopMoedas] = useState<string | null>(initialData?.errorTopMoedas ?? null);

  const fetchWithSessionRetry = useCallback(async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  const carregarPerformance = useCallback(async () => {
    ...
    try {
      const response = await fetchWithSessionRetry('/api/crypto/performance');
      const result = await ensureJsonApiResponse(response);
  ...
  const carregarTopMoedas = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchWithSessionRetry(topMoedasUrl);
      if (response.status === 429) throw new Error("Limite de requisições excedido (CoinGecko).");
      if (!response.ok) {
           const errorData = await ensureJsonApiResponse(response).catch(() => ({}));
      ...
      const marketDataMap: MarketDataMap = await ensureJsonApiResponse(response);
  ...
  useEffect(() => {
    ...
    console.log('[HomePage] useEffect inicial disparado.');
    void carregarPerformance();
    void carregarTopMoedas();
  }, [carregarPerformance, carregarTopMoedas, initialData]);
