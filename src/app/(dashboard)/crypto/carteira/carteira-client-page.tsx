...
export interface CarteiraInitialData {
  summary: PerformanceApiResponse["summary"];
  performanceMap: Record<string, PerformanceMetrics>;
  operacoes: Operacao[];
  carteira: CarteiraData | null;
  carteiras: PortfolioOption[];
...
interface CryptoCarteiraPageProps {
  initialData?: CarteiraInitialData;
}

export default function CryptoCarteiraPage({ initialData }: CryptoCarteiraPageProps) {
  const router = useRouter();
  const [loading, setLoading] = useState(!initialData);
  const [error, setError] = useState<string | null>(initialData?.error ?? null);
  const [warning, setWarning] = useState<string | null>(initialData?.warning ?? null);
  ...
  const [performanceMap, setPerformanceMap] = useState<Record<string, PerformanceMetrics>>(
    initialData?.performanceMap ?? {}
  );
  const [operacoes, setOperacoes] = useState<Operacao[]>(initialData?.operacoes ?? []);
  const [carteira, setCarteira] = useState<CarteiraData | null>(initialData?.carteira ?? null);
  const [carteiras, setCarteiras] = useState<PortfolioOption[]>(initialData?.carteiras ?? []);
  const [selectedPortfolioId, setSelectedPortfolioId] = useState<string | null>(
    initialData?.selectedPortfolioId ?? initialData?.carteira?.id ?? null
  );
  const [aportes, setAportes] = useState<AporteData[]>(initialData?.aportes ?? []);
  const [snapshots, setSnapshots] = useState<SnapshotData[]>(initialData?.snapshots ?? []);
  ...
  const loadData = useCallback(async (portfolioId?: string | null) => {
    setLoading(true);
    setError(null);
    setWarning(null);

    try {
      const carteiraQueryParam = portfolioId ? `?carteira_id=${encodeURIComponent(portfolioId)}` : "";
      const carteiraRes = await fetch(`/api/crypto/carteira${carteiraQueryParam}`, { cache: "no-store" });
      const carteiraJson = await ensureJsonApiResponse<Record<string, any>>(carteiraRes, { carteira: null, aportes: [] });
      const carteirasAtivas = Array.isArray(carteiraJson?.carteiras) ? carteiraJson.carteiras : [];
      const carteiraAtual: CarteiraData | null =
        carteiraRes.ok
          ? carteiraJson.carteira ??
            (portfolioId ? carteirasAtivas.find((item: PortfolioOption) => item.id === portfolioId) : null) ??
            carteirasAtivas[0] ??
            null
          : null;
      const carteiraAtualId = carteiraAtual?.id ?? null;
      setCarteira(carteiraAtual);
      setCarteiras(carteirasAtivas);
      setSelectedPortfolioId(carteiraAtualId);
      setAportes(Array.isArray(carteiraJson?.aportes) ? carteiraJson.aportes : []);

      const carteiraQuery = carteiraAtualId ? `?carteira_id=${encodeURIComponent(carteiraAtualId)}` : "";
      ...
      const perfJson = await ensureJsonApiResponse<PerformanceApiResponse>(perfRes, {
        performance: {},
        summary: {
          totalRealizado: 0,
          totalNaoRealizado: 0,
      ...
      const opsJson = await ensureJsonApiResponse<Operacao[]>(opsRes, []);
      const snapJson = await ensureJsonApiResponse<Record<string, any>>(snapRes, { snapshots: [] });

      if (!perfJson?.summary || !perfJson?.performance) {
        throw new Error("Resposta inválida ao carregar performance.");
      }
  ...
  useEffect(() => {
    ...
    setWarning(initialData.warning ?? null);
    setSummary(
      initialData.summary ?? {
        totalRealizado: 0,
        totalNaoRealizado: 0,
    ...
    setPerformanceMap(initialData.performanceMap ?? {});
    setOperacoes(initialData.operacoes ?? []);
    setCarteira(initialData.carteira ?? null);
    setCarteiras(initialData.carteiras ?? []);
    setSelectedPortfolioId(initialData.selectedPortfolioId ?? initialData.carteira?.id ?? null);
    setAportes(initialData.aportes ?? []);
  ...
  const handlePortfolioChange = (portfolioId: string) => {
    ...
    const nextUrl = `/crypto/carteira?carteira_id=${encodeURIComponent(portfolioId)}`;
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-2">
          <Select
            ...
            disabled={loading || carteiras.length === 0}
          >
            <SelectTrigger className="w-full min-w-[220px]">
              <SelectValue placeholder="Selecione um portfolio" />
            </SelectTrigger>
            <SelectContent>
              {carteiras.map((portfolio) => (
